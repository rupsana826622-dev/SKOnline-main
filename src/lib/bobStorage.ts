...
function mapBobCustomerToDb(r: BobCustomerRecord, tenantId: string = "bob_csp"): Record<string, any> {
  return {
    id: r.id,
...
function mapDbToBobCustomer(row: any): BobCustomerRecord {
  let extra: any = {};
  if (row.family_id) {
    try {
  ...
  return {
    ...
    passbookIssuedAt: row.passbook_issued_at || null,
    atmIssued: !!row.atm_issued,
    atmIssuedAt: row.atm_issued_at || null,
    notes: row.notes || "",
...
export async function addBobCustomer(record: BobCustomerRecord): Promise<{ error: any; data?: any }> {
  ...
  try {
    ...
    // 1. Primary insert to public.bob_customers strictly scoped to tenant_id
    try {
    ...
    // 2. Also insert to customers table scoped to tenant_code: 'bob_csp'
    try {
      await supabase.from("customers").upsert([payload]);
    } catch (dbErr) {
      console.warn("Error inserting to customers table:", dbErr);
    }

    return { error: null, data: payload };
  } catch (err: any) {
    console.warn("Network error saving BOB customer to Supabase:", err);
    return { error: null };
  }
}
...
export async function updateBobCustomer(id: string, updates: Partial<BobCustomerRecord>): Promise<{ error: any }> {
  ...
  try {
    ...
    // Update bob_customers strictly scoped to tenant_id
    try {
    ...
    console.warn("Error syncing BOB update to Supabase:", err);
  }
...
export async function deleteBobCustomer(id: string): Promise<{ error: any }> {
  ...
  try {
    ...
    console.warn("Error deleting BOB record from Supabase:", err);
    return { error: null };
  }
}
...
export async function syncBobFromSupabase(): Promise<void> {
  ...
  try {
    ...
    // 2. Sync Records strictly scoped from bob_customers with tenant_id = currentTenant.id
